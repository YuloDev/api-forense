...
Migrado a la nueva arquitectura hexagonal
...
def preprocess_for_ocr_robust(img_bgr):
    """Preprocesamiento robusto para OCR de PDFs escaneados"""
...
def is_scanned_image_pdf(pdf_bytes: bytes, extracted_text: str) -> bool:
    """Detecta si un PDF es escaneado (poco texto + imágenes)."""
...
def extract_barcodes_from_image(img_array):
    """Extrae códigos de barras usando pyzbar"""
    if zbar_decode:
        try:
            for bc in zbar_decode(cv2.cvtColor(img_array, cv2.COLOR_BGR2GRAY)):
...
def extract_page_factura_data(text: str) -> Dict[str, Any]:
    """Extrae datos de factura de una página específica"""
    # Patrones de extracción robustos
    r_ruc = re.search(r'(?:R\.?U\.?C\.?:?|RUC[:\s]*)\s*([0-9]{13})', text, re.I)
    r_razon = re.search(r'(Raz[oó]n Social.*?:\s*)([^\n\r]+)', text, re.I)
    r_fecha = re.search(r'(?:Fecha(?: de Emisi[oó]n)?[:\s]*)\s*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})', text, re.I)
    r_num = re.search(r'(?:No\.?|N[uú]mero(?: de factura)?)[:\s]*([0-9]{3}[-–][0-9]{3}[-–][0-9]{6,9})', text, re.I)
    r_clave = re.search(r'(?:Clave.*Acceso|N[uú]mero de autorizaci[oó]n)[:\s]*([0-9\s]{45,60})', text, re.I)
    ...
    candidates += re.findall(r'\d[\d\s]{47,70}\d', text)
    ...
    total = None
    m_total = re.search(r'(VALOR\s+TOTAL|TOTAL)\D{0,30}([0-9]+[.,][0-9]{2})', text, re.I)
    if m_total:
...
def extraer_datos_factura_pdf_robust(pdf_bytes: bytes, lang='spa+eng') -> Dict[str, Any]:
    """
    ...
    for page in doc:
        ...
        barcode_key = extract_barcodes_from_image(img)
        if barcode_key:
            claves_barcodes.append(barcode_key)
