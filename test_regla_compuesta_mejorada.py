...
"""
Test de la regla compuesta mejorada: Texto sintético aplanado + Ruido/Bordes
"""
...
def test_regla_compuesta_mejorada():
    """Test de la regla compuesta mejorada"""
    print("🔬 TEST REGLA COMPUESTA MEJORADA: TEXTO SINTÉTICO + RUIDO/BORDES")
    print("=" * 80)
            ...
            if 'riesgo' in result:
                ...
                for i, check in enumerate(prioritarias):
                    ...
                    if check.get('check') == "Texto sintético aplanado + Ruido/Bordes":
                        ...
                        print(f"      ✅ REGLA COMPUESTA MEJORADA ENCONTRADA EN PRIORITARIAS")
                        print(f"      Nivel: {detalle.get('nivel', 'N/A')}")
                        print(f"      Score parcial: {detalle.get('score_parcial', 'N/A')}")
                        print(f"      Policy: {detalle.get('policy', 'N/A')}")
                        ...
                        if metricas:
                            ...
                            print(f"        - Edge density: {metricas.get('edge_density', 'N/A')}")
                            print(f"        - Es localizado: {metricas.get('es_localizado', 'N/A')}")
                            print(f"        - Bordes agrupados: {metricas.get('bordes_agrupados', 'N/A')}")
                            print(f"        - Densidad bordes sospechosos: {metricas.get('densidad_bordes_sospechosos', 'N/A')}")
                            print(f"        - ELA regional: {metricas.get('ela_pct_reg', 'N/A')}")
                            print(f"        - ELA global: {metricas.get('ela_pct_global', 'N/A')}")
                        
                        # Mostrar umbrales
                        umbrales = detalle.get('umbrales', {})
                        if umbrales:
                            print(f"      Umbrales aplicados:")
                            print(f"        - TH_BOXES: {umbrales.get('TH_BOXES', 'N/A')}")
                            print(f"        - TH_SW_CV: {umbrales.get('TH_SW_CV', 'N/A')}")
                            print(f"        - TH_HALO: {umbrales.get('TH_HALO', 'N/A')}")
                            print(f"        - TH_OUTLIER: {umbrales.get('TH_OUTLIER', 'N/A')}")
                            print(f"        - TH_DENSLOC: {umbrales.get('TH_DENSLOC', 'N/A')}")
                            print(f"        - TH_ELA_REG: {umbrales.get('TH_ELA_REG', 'N/A')}")
